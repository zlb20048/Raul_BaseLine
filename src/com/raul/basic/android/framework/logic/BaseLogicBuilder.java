...
/**
 * 抽象的logic建造类，用戶緩存logic和集中增加和移除hander
 */
abstract public class BaseLogicBuilder implements ILogicBuilder {
	...
	/**
	 * 初始化所有的logic对象 在该方法遍历缓存中所有的logic对象，执行所有logic的init方法
	 * 
	 * @param context
	 *            系统的context对象
	 */
	...
	/**
	 * 根据接口类移除缓存中logic对象 移除缓存中的logic对象 根据接口类,可以将缓存中集中管理的logic对象移除
	 * 
	...
	/**
	 * 根据接口类注册对应的logic对象到管理缓存中 一般在子类的init方法中被执行，否则logic对象的init方法就不被调用
	 * 
	 * @param interfaceClass
	 *            logic的接口类
	 * @param logic
	...
	protected void registerLogic(Class<?> interfaceClass, ILogic logic) {
		if (!mLogicCache.containsKey(interfaceClass.getName())) {
			...
			if (isInterface(logicClass, interfaceName)
				...
				Log.i(TAG, "Register logic(" + logicClass.getName()
						+ ") successful.");
			} else {
	...
	/**
	 * 对缓存中的所有logic对象增加hander 对缓存中的所有logic对象增加hander，在该UI的onCreated时被框架执行，
	...
	/**
	 * 对缓存中的所有logic对象移除hander对象 在该UI的onDestory时被框架执行，如果该logic对象
