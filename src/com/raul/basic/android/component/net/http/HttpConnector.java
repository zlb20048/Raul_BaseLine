...
/**
 * HTTP连接类，提供连接的功能 负责发起HTTP连接、获取网络数据
 * 
 * @author xiaomiaomiao
 * 
 */
public class HttpConnector {
	...
	public static Response connect(Request request) {
		...
		try {
			...
			e.printStackTrace();
			Log.d(TAG, "HttpConnector exception!");
			...
			try {
				...
				Log.d(TAG, "IOException getrespCode:" + responseCode);
			...
			Log.d(TAG, "HttpConnector IOException ......!");
	...
	private static void setResponseData(Request request, Response response,
		...
		if (!request.isNeedByte()) {
			...
			Log.d(TAG, "response data : [ is byte data ]");
		}
	}
