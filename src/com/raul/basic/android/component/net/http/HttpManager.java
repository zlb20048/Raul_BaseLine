...
import com.raul.basic.android.component.net.http.Request.ContentType;
import com.raul.basic.android.component.net.http.Request.RequestMethod;
import com.raul.basic.android.component.net.http.Response.ResponseCode;
...
public abstract class HttpManager {
	 ...
	 *            请求标识，不同请求定义不同的标识位
	 * @param sendData
	 *            请求参数，由调用者自己进行封装，其中的key-value自己去定义
	 * @param httpListener
	 *            HTTP监听器，服务器返回结果会组装成Response对象并通过onResult()方法回调给调用者
	 */
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				case InternalError:
					...
					Log.d(TAG,
							"response:result code[" + response.getResultCode()
									+ "] result desc["
									+ response.getResultDesc() + "]");
					if (response.getResponseCode() == ResponseCode.Succeed) {
						Log.d(TAG, "handledata()");
						response.setObj(handleData(action, sendData, response));
					}
					break;
				default:
					break;
			...
			if (getContentType(action) == ContentType.JSON
				...
				try {
					...
					if (rootJsonObj.has("Result")) {
						JSONObject resultObj = rootJsonObj
