...
import com.raul.basic.android.util.JsonUtils;
...
public final class LocationService {
	...
	public LocationInfo getLocationInfo() {
		...
		if (hasStartListener) {
			if (locationInfo.getLocation() == null) {
				...
				while (counter++ < 20) {
					...
					if (mLocation != null) {
						Log.d(TAG, "got location by provider");
		...
		Log.d(TAG, "totalTime = " + (System.currentTimeMillis() - init));
		return locationInfo;
	}
	...
	private void startListenLocation(String locationProvider) {
		mLocationMgr.requestLocationUpdates(locationProvider, MIN_TIME,
				MIN_INSTANCE, mLocationListener);
	}
	...
	private LocationInfo getLocationByCellOrWifi() {
		...
		try {
			...
			Log.d(TAG, "getLocationInfo: Location Send*****" + strJson);
	...
	public String getAddress(double lat, double lon) {
		...
		String url = String
				.format("http://maps.google.cn/maps/geo?output=csv&hl=%s&key=abcdef&q=%s,%s",
						parseLanguage, lat, lon);
	...
	/**
	 * 
	 * 判断/system/framework/中是否有com.google.android.maps.jar [功能详细描述]
	 * 
	 * @return boolean
	 */
